...
document.querySelectorAll('.slider-controls li').forEach((indicator, ind) => {
  indicator.addEventListener('click', () => {
    ...
    slider.style.transform = `translate(${
      (sectionIndex * -100) / numberOfSlide
    }%)`
  })
})
...
function openModal(date) {
  ...
  if (eventForDay) {
    ...
    const isoDateString = `${dateParts[2]}-${
      dateParts[1].length === 1 ? '0' : ''
    }${dateParts[1]}-${dateParts[0].length === 1 ? '0' : ''}${dateParts[0]}`
...
function initButtons() {
  ...
  document
    .getElementById('cancelVisitorZone')
  ...
  document
    .getElementById('cancelAuthentificationButton')
    .addEventListener('click', closeModal)
...
function postForms() {
  ...
  const nom = document.getElementById('nameInput')
  const email = document.getElementById('emailInput')
  const date1 = document.getElementById('firstDate')
  const date2 = document.getElementById('secondDate')
